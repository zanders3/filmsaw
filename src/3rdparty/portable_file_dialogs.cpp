...
#undef PFD_OSX_ICON
    ...
    case choice::yes_no_cancel:
      command.insert(command.end(),
                     {"--question", "--switch", "--extra-button=Cancel", "--extra-button=No", "--extra-button=Yes"});
      break;
    case choice::retry_cancel:
...
int pfd_open_dialog(const char* title, const char** filters, int numfilters, char* filepath_out, int filepath_out_len) {
  ...
  for (int i = 0; i < numfilters; i++) {
    filtersvec.push_back(std::string(filters[i]));
  }
  std::string titlestr = std::string(title);
  std::vector<std::string> result = pfd::open_file::open_file(titlestr, ".", filtersvec, false).result();
  if (result.size() > 0) {
    snprintf(filepath_out, filepath_out_len, "%.*s", (int)result[0].size(), result[0].c_str());
    return 1;
  } else {
    filepath_out[0] = 0;
    return 0;
...
int pfd_save_dialog(const char* title, const char* defaultpath, const char** filters, int numfilters,
  ...
  std::string titlestr = std::string(title);
  std::string result = pfd::save_file::save_file(titlestr, defaultpath, filtersvec).result();
  snprintf(filepath_out, filepath_out_len, "%.*s", (int)result.size(), result.c_str());
