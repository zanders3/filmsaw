...
typedef struct sapp_desc {
  ...
  bool html5_canvas_resize; // if true, the HTML5 canvas size is set to sapp_desc.width/height, otherwise canvas size is
                            // tracked
  bool html5_preserve_drawing_buffer; // HTML5 only: whether to preserve default framebuffer content between frames
  bool html5_premultiplied_alpha;     // HTML5 only: whether the rendered pixels use premultiplied alpha convention
  bool
      html5_ask_leave_site; // initial state of the internal html5_ask_leave_site flag (see sapp_html5_ask_leave_site())
...
_SOKOL_PRIVATE void _sapp_setup_default_icon(void) {
  ...
  const uint32_t colors[8] = {
      0xFF4370FF, 0xFF26A7FF, 0xFF58EEFF, 0xFF57E1D4, 0xFF65CC9C, 0xFF6ABB66, 0xFFF5A542, 0xFFC2577E,
  };
...
_SOKOL_PRIVATE void _sapp_macos_init_cursors(void) {
  ...
  _sapp.macos.cursors[SAPP_MOUSECURSOR_RESIZE_EW] = [NSCursor respondsToSelector:@selector(_windowResizeEastWestCursor)]
                                                        ? [NSCursor _windowResizeEastWestCursor]
                                                        : [NSCursor resizeLeftRightCursor];
  _sapp.macos.cursors[SAPP_MOUSECURSOR_RESIZE_NS] =
      [NSCursor respondsToSelector:@selector(_windowResizeNorthSouthCursor)] ? [NSCursor _windowResizeNorthSouthCursor]
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  if ([pboard.types containsObject:NSPasteboardTypeFileURL]) {
    ...
    _sapp.drop.num_files = ((int)pboard.pasteboardItems.count > _sapp.drop.max_files) ? _sapp.drop.max_files
    ...
    for (int i = 0; i < _sapp.drop.num_files; i++) {
      ...
      if (!_sapp_strcpy(fileUrl.standardizedURL.path.UTF8String, _sapp_dropped_file_path_ptr(i),
...
- (void)keyUp:(NSEvent*)event {
  _sapp_macos_key_event(SAPP_EVENTTYPE_KEY_UP, _sapp_translate_key(event.keyCode), event.isARepeat,
...
- (void)flagsChanged:(NSEvent*)event {
  ...
  if (key_code != SAPP_KEYCODE_INVALID) {
    _sapp_macos_key_event(down ? SAPP_EVENTTYPE_KEY_DOWN : SAPP_EVENTTYPE_KEY_UP, key_code, false,
                          _sapp_macos_mods(event));
  }
}
@end
...
EMSCRIPTEN_KEEPALIVE void _sapp_emsc_invoke_fetch_cb(int index, int success, int error_code,
                                                     _sapp_html5_fetch_callback callback, uint32_t fetched_size,
                                                     void* buf_ptr, uint32_t buf_size, void* user_data) {
...
_SOKOL_PRIVATE EM_BOOL _sapp_emsc_pointerlockchange_cb(int emsc_type,
...
_SOKOL_PRIVATE const _sapp_gl_fbconfig* _sapp_gl_choose_fbconfig(const _sapp_gl_fbconfig* desired,
...
static inline void _sapp_d3d11_ResolveSubresource(ID3D11DeviceContext* self, ID3D11Resource* pDstResource,
...
_SOKOL_PRIVATE void _sapp_d3d11_create_device_and_swapchain(void) {
  ...
  if (!SUCCEEDED(hr)) {
    ...
    // ===
    // D3D11CreateDevice: Flags (0x2) were specified which require the D3D11 SDK Layers for Windows 10, but they are not
    // present on the system. These flags must be removed, or the Windows 10 SDK must be installed. Flags include:
    // D3D11_CREATE_DEVICE_DEBUG
    // ===
    //
    // ...just retry with the DEBUG flag switched off
    SOKOL_LOG("sokol_app.h: D3D11CreateDeviceAndSwapChain() with D3D11_CREATE_DEVICE_DEBUG failed, retrying without "
              "debug flag.\n");
    create_flags &= ~D3D11_CREATE_DEVICE_DEBUG;
...
_SOKOL_PRIVATE void _sapp_d3d11_resize_default_render_target(void) {
  if (_sapp.d3d11.swap_chain) {
    ...
    _sapp_dxgi_ResizeBuffers(_sapp.d3d11.swap_chain, _sapp.d3d11.swap_chain_desc.BufferCount,
                             (UINT)_sapp.framebuffer_width, (UINT)_sapp.framebuffer_height, DXGI_FORMAT_B8G8R8A8_UNORM,
                             0);
...
_SOKOL_PRIVATE void _sapp_d3d11_present(bool do_not_wait) {
  ...
  if (_sapp.sample_count > 1) {
    ...
    _sapp_d3d11_ResolveSubresource(_sapp.d3d11.device_context, (ID3D11Resource*)_sapp.d3d11.rt, 0,
                                   (ID3D11Resource*)_sapp.d3d11.msaa_rt, 0, DXGI_FORMAT_B8G8R8A8_UNORM);
  }
  UINT flags = 0;
...
_SOKOL_PRIVATE void _sapp_wgl_init(void) {
  ...
  _sapp.wgl.msg_hwnd =
      CreateWindowExW(WS_EX_OVERLAPPEDWINDOW, L"SOKOLAPP", L"sokol-app message window",
...
_SOKOL_PRIVATE int _sapp_wgl_find_pixel_format(void) {
  ...
  _sapp_gl_fbconfig* usable_configs =
      (_sapp_gl_fbconfig*)_sapp_malloc_clear((size_t)native_count * sizeof(_sapp_gl_fbconfig));
...
_SOKOL_PRIVATE void _sapp_win32_lock_mouse(bool lock) {
  ...
  if (_sapp.mouse.locked) {
    ...
    */
    RECT client_rect = {_sapp.win32.mouse_locked_x, _sapp.win32.mouse_locked_y, _sapp.win32.mouse_locked_x,
...
_SOKOL_PRIVATE void _sapp_win32_key_event(sapp_event_type type, int vk, bool repeat) {
  if (_sapp_events_enabled() && (vk < SAPP_MAX_KEYCODES)) {
    ...
    if (_sapp.clipboard.enabled && (type == SAPP_EVENTTYPE_KEY_DOWN) && (_sapp.event.modifiers == SAPP_MODIFIER_CTRL) &&
  ...
  */
  ...
  if (fn_getdpiforwindow) {
    ...
    SetWindowPos(hWnd, 0, proposed_win_rect->left, proposed_win_rect->top,
                 proposed_win_rect->right - proposed_win_rect->left, proposed_win_rect->bottom - proposed_win_rect->top,
  ...
  if (_sapp.d3d11.use_dxgi_frame_stats) {
    ...
    if (SUCCEEDED(hr)) {
      if (dxgi_stats.SyncRefreshCount != _sapp.d3d11.sync_refresh_count) {
        ...
        const uint64_t now = (uint64_t)_sapp_int64_muldiv(qpc.QuadPart - _sapp.timing.timestamp.win.start.QuadPart,
...
_SOKOL_PRIVATE void _sapp_win32_create_window(void) {
  ...
  wndclassw.hIcon = LoadIcon(wndclassw.hInstance, MAKEINTRESOURCE(IDI_ICON1));
...
_SOKOL_PRIVATE void _sapp_win32_set_icon(const sapp_icon_desc* icon_desc, int num_images) {
  ...
  int sml_img_index = _sapp_image_bestmatch(icon_desc->images, num_images, GetSystemMetrics(SM_CXSMICON),
...
_SOKOL_PRIVATE void _sapp_uwp_update_cursor(sapp_mouse_cursor cursor, bool shown) {
  ...
  if (shown) {
    switch (cursor) {
    case SAPP_MOUSECURSOR_ARROW:
      uwp_cursor = CoreCursor(CoreCursorType::Arrow, 0);
      break;
    case SAPP_MOUSECURSOR_IBEAM:
      uwp_cursor = CoreCursor(CoreCursorType::IBeam, 0);
      break;
    case SAPP_MOUSECURSOR_CROSSHAIR:
      uwp_cursor = CoreCursor(CoreCursorType::Cross, 0);
      break;
    case SAPP_MOUSECURSOR_POINTING_HAND:
      uwp_cursor = CoreCursor(CoreCursorType::Hand, 0);
      break;
    case SAPP_MOUSECURSOR_RESIZE_EW:
      uwp_cursor = CoreCursor(CoreCursorType::SizeWestEast, 0);
      break;
    case SAPP_MOUSECURSOR_RESIZE_NS:
      uwp_cursor = CoreCursor(CoreCursorType::SizeNorthSouth, 0);
      break;
    case SAPP_MOUSECURSOR_RESIZE_NWSE:
      uwp_cursor = CoreCursor(CoreCursorType::SizeNorthwestSoutheast, 0);
      break;
    case SAPP_MOUSECURSOR_RESIZE_NESW:
      uwp_cursor = CoreCursor(CoreCursorType::SizeNortheastSouthwest, 0);
      break;
    case SAPP_MOUSECURSOR_RESIZE_ALL:
      uwp_cursor = CoreCursor(CoreCursorType::SizeAll, 0);
      break;
    case SAPP_MOUSECURSOR_NOT_ALLOWED:
      uwp_cursor = CoreCursor(CoreCursorType::UniversalNo, 0);
      break;
    default:
      uwp_cursor = CoreCursor(CoreCursorType::Arrow, 0);
      break;
...
_SOKOL_PRIVATE uint32_t _sapp_uwp_mods(winrt::Windows::UI::Core::CoreWindow const& sender_window) {
  ...
  if (((sender_window.GetKeyState(VirtualKey::LeftWindows) & CoreVirtualKeyStates::Down) ==
       CoreVirtualKeyStates::Down) ||
      ((sender_window.GetKeyState(VirtualKey::RightWindows) & CoreVirtualKeyStates::Down) ==
       CoreVirtualKeyStates::Down)) {
    mods |= SAPP_MODIFIER_SUPER;
  }
...
_SOKOL_PRIVATE void _sapp_uwp_mouse_event(sapp_event_type type, sapp_mousebutton btn,
...
_SOKOL_PRIVATE void _sapp_uwp_scroll_event(float delta, bool horizontal,
                                           winrt::Windows::UI::Core::CoreWindow const& sender_window) {
  if (_sapp_events_enabled()) {
    _sapp_init_event(SAPP_EVENTTYPE_MOUSE_SCROLL);
    _sapp.event.modifiers = _sapp_uwp_mods(sender_window);
...
_SOKOL_PRIVATE void _sapp_uwp_extract_mouse_button_events(winrt::Windows::UI::Core::CoreWindow const& sender,
...
_SOKOL_PRIVATE void _sapp_uwp_key_event(sapp_event_type type, winrt::Windows::UI::Core::CoreWindow const& sender_window,
...
protected:
  ...
  void OnPointerEntered(winrt::Windows::UI::Core::CoreWindow const& sender,
                        winrt::Windows::UI::Core::PointerEventArgs const& args);
  void OnPointerExited(winrt::Windows::UI::Core::CoreWindow const& sender,
                       winrt::Windows::UI::Core::PointerEventArgs const& args);
  ...
  void OnPointerMoved(winrt::Windows::UI::Core::CoreWindow const& sender,
                      winrt::Windows::UI::Core::PointerEventArgs const& args);
  void OnPointerWheelChanged(winrt::Windows::UI::Core::CoreWindow const& sender,
                             winrt::Windows::UI::Core::PointerEventArgs const& args);
...
void DeviceResources::CreateDeviceResources() {
  ...
  if (FAILED(hr)) {
    ...
    winrt::check_hresult(D3D11CreateDevice(nullptr,
                                           ...
                                           device.put(), &m_d3dFeatureLevel, context.put()));
  }
...
void DeviceResources::CreateWindowSizeDependentResources() {
  ...
  if (m_swapChain != nullptr) {
    ...
    DXGI_SCALING scaling =
        (_sapp.uwp.dpi.content_scale == _sapp.uwp.dpi.window_scale) ? DXGI_SCALING_NONE : DXGI_SCALING_STRETCH;
    ...
    winrt::check_hresult(dxgiFactory->CreateSwapChainForCoreWindow(
        m_d3dDevice.get(), m_window.get().as<::IUnknown>().get(), &swapChainDesc, nullptr, swapChain.put()));
  ...
  winrt::check_hresult(
      m_d3dDevice->CreateRenderTargetView1(m_d3dRenderTarget.get(), nullptr, m_d3dRenderTargetView.put()));

  // Create MSAA texture and view if needed
  if (_sapp.sample_count > 1) {
    CD3D11_TEXTURE2D_DESC1 msaaTexDesc(
        DXGI_FORMAT_B8G8R8A8_UNORM, lround(m_d3dRenderTargetSize.Width), lround(m_d3dRenderTargetSize.Height),
        1, // arraySize
        1, // mipLevels
        D3D11_BIND_RENDER_TARGET, D3D11_USAGE_DEFAULT,
        0, // cpuAccessFlags
        _sapp.sample_count, _sapp.sample_count > 1 ? D3D11_STANDARD_MULTISAMPLE_PATTERN : 0);
    winrt::check_hresult(m_d3dDevice->CreateTexture2D1(&msaaTexDesc, nullptr, m_d3dMSAARenderTarget.put()));
    winrt::check_hresult(
        m_d3dDevice->CreateRenderTargetView1(m_d3dMSAARenderTarget.get(), nullptr, m_d3dMSAARenderTargetView.put()));
  }

  // Create a depth stencil view for use with 3D rendering if needed.
  CD3D11_TEXTURE2D_DESC1 depthStencilDesc(
      DXGI_FORMAT_D24_UNORM_S8_UINT, lround(m_d3dRenderTargetSize.Width), lround(m_d3dRenderTargetSize.Height),
  ...
  winrt::check_hresult(
      m_d3dDevice->CreateDepthStencilView(m_d3dDepthStencil.get(), nullptr, m_d3dDepthStencilView.put()));
...
void DeviceResources::Present() {
  ...
  if (_sapp.sample_count > 1) {
    m_d3dContext->ResolveSubresource(m_d3dRenderTarget.get(), 0, m_d3dMSAARenderTarget.get(), 0,
...
void App::Run() {
  ...
  while (true) {
    if (m_windowVisible) {
      ...
      winrt::Windows::UI::Core::CoreWindow::GetForCurrentThread().Dispatcher().ProcessEvents(
          winrt::Windows::UI::Core::CoreProcessEventsOption::ProcessOneAndAllPending);
    }
  }
}
...
// Application lifecycle event handlers.
void App::OnActivated(winrt::Windows::ApplicationModel::Core::CoreApplicationView const& applicationView,
                      winrt::Windows::ApplicationModel::Activation::IActivatedEventArgs const& args) {
  _SOKOL_UNUSED(args);
  ...
  winrt::Windows::UI::ViewManagement::ApplicationView::PreferredLaunchWindowingMode(
      winrt::Windows::UI::ViewManagement::ApplicationViewWindowingMode::PreferredLaunchViewSize);
...
void App::OnSuspending(winrt::Windows::Foundation::IInspectable const& sender,
                       winrt::Windows::ApplicationModel::SuspendingEventArgs const& args) {
  _SOKOL_UNUSED(sender);
  _SOKOL_UNUSED(args);
...
void App::OnWindowSizeChanged(winrt::Windows::UI::Core::CoreWindow const& sender,
                              winrt::Windows::UI::Core::WindowSizeChangedEventArgs const& args) {
  _SOKOL_UNUSED(args);
...
void App::OnVisibilityChanged(winrt::Windows::UI::Core::CoreWindow const& sender,
                              winrt::Windows::UI::Core::VisibilityChangedEventArgs const& args) {
  _SOKOL_UNUSED(sender);
...
void App::OnBackRequested(winrt::Windows::Foundation::IInspectable const& sender,
                          winrt::Windows::UI::Core::BackRequestedEventArgs const& args) {
  _SOKOL_UNUSED(sender);
  args.Handled(true);
}
...
void App::OnCharacterReceived(winrt::Windows::UI::Core::CoreWindow const& sender,
                              winrt::Windows::UI::Core::CharacterReceivedEventArgs const& args) {
  _sapp_uwp_char_event(args.KeyCode(), args.KeyStatus().WasKeyDown, sender);
}

void App::OnPointerEntered(winrt::Windows::UI::Core::CoreWindow const& sender,
                           winrt::Windows::UI::Core::PointerEventArgs const& args) {
  _SOKOL_UNUSED(args);
...
void App::OnPointerExited(winrt::Windows::UI::Core::CoreWindow const& sender,
                          winrt::Windows::UI::Core::PointerEventArgs const& args) {
  _SOKOL_UNUSED(args);
...
void App::OnPointerPressed(winrt::Windows::UI::Core::CoreWindow const& sender,
                           winrt::Windows::UI::Core::PointerEventArgs const& args) {
  _sapp_uwp_extract_mouse_button_events(sender, args);
}

// NOTE: for some reason this event handler is never called??
void App::OnPointerReleased(winrt::Windows::UI::Core::CoreWindow const& sender,
                            winrt::Windows::UI::Core::PointerEventArgs const& args) {
  _sapp_uwp_extract_mouse_button_events(sender, args);
}

void App::OnPointerMoved(winrt::Windows::UI::Core::CoreWindow const& sender,
                         winrt::Windows::UI::Core::PointerEventArgs const& args) {
...
void App::OnPointerWheelChanged(winrt::Windows::UI::Core::CoreWindow const& sender,
                                winrt::Windows::UI::Core::PointerEventArgs const& args) {
  auto properties = args.CurrentPoint().Properties();
  _sapp_uwp_scroll_event((float)properties.MouseWheelDelta(), properties.IsHorizontalMouseWheel(), sender);
}

void App::OnDpiChanged(winrt::Windows::Graphics::Display::DisplayInformation const& sender,
                       winrt::Windows::Foundation::IInspectable const& args) {
  // NOTE: UNTESTED
  _SOKOL_UNUSED(args);
...
void App::OnOrientationChanged(winrt::Windows::Graphics::Display::DisplayInformation const& sender,
                               winrt::Windows::Foundation::IInspectable const& args) {
  // NOTE: UNTESTED
  _SOKOL_UNUSED(args);
...
void App::OnDisplayContentsInvalidated(winrt::Windows::Graphics::Display::DisplayInformation const& sender,
                                       winrt::Windows::Foundation::IInspectable const& args) {
  // NOTE: UNTESTED
  _SOKOL_UNUSED(args);
  _SOKOL_UNUSED(sender);
...
#if defined(SOKOL_GLES3)
    EGL_RENDERABLE_TYPE,
    _sapp.desc.gl_force_gles2 ? EGL_OPENGL_ES2_BIT : EGL_OPENGL_ES3_BIT,
#else
    EGL_RENDERABLE_TYPE,
    EGL_OPENGL_ES2_BIT,
#endif
  ...
  case _SOKOL_ANDROID_MSG_SET_INPUT_QUEUE:
    ...
    if (_sapp.android.current.input != _sapp.android.pending.input) {
      ...
      if (_sapp.android.pending.input != NULL) {
        AInputQueue_attachLooper(_sapp.android.pending.input, _sapp.android.looper, ALOOPER_POLL_CALLBACK,
...
_SOKOL_PRIVATE void* _sapp_android_loop(void* arg) {
  ...
  ALooper_addFd(_sapp.android.looper, _sapp.android.pt.read_from_main_fd, ALOOPER_POLL_CALLBACK, ALOOPER_EVENT_INPUT,
...
_SOKOL_PRIVATE void _sapp_x11_init_extensions(void) {
  ...
  if (XQueryExtension(_sapp.x11.display, "XInputExtension", &_sapp.x11.xi.major_opcode, &_sapp.x11.xi.event_base,
...
_SOKOL_PRIVATE void _sapp_glx_init() {
  ...
  if (!_sapp.glx.GetFBConfigs || !_sapp.glx.GetFBConfigAttrib || !_sapp.glx.GetClientString ||
      !_sapp.glx.QueryExtension || !_sapp.glx.QueryVersion || !_sapp.glx.DestroyContext || !_sapp.glx.MakeCurrent ||
      !_sapp.glx.SwapBuffers || !_sapp.glx.QueryExtensionsString || !_sapp.glx.CreateWindow ||
      !_sapp.glx.DestroyWindow || !_sapp.glx.GetProcAddress || !_sapp.glx.GetProcAddressARB ||
...
_SOKOL_PRIVATE void _sapp_x11_create_standard_cursor(sapp_mouse_cursor cursor, const char* name, const char* theme,
                                                     int size, uint32_t fallback_native) {
  SOKOL_ASSERT((cursor >= 0) && (cursor < _SAPP_MOUSECURSOR_NUM));
...
_SOKOL_PRIVATE void _sapp_x11_update_window_title(void) {
  Xutf8SetWMProperties(_sapp.x11.display, _sapp.x11.window, _sapp.window_title, _sapp.window_title, NULL, 0, NULL, NULL,
                       NULL);
  XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.NET_WM_NAME, _sapp.x11.UTF8_STRING, 8, PropModeReplace,
                  (unsigned char*)_sapp.window_title, strlen(_sapp.window_title));
  XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.NET_WM_ICON_NAME, _sapp.x11.UTF8_STRING, 8,
                  PropModeReplace, (unsigned char*)_sapp.window_title, strlen(_sapp.window_title));
  XFlush(_sapp.x11.display);
}
...
_SOKOL_PRIVATE void _sapp_x11_set_icon(const sapp_icon_desc* icon_desc, int num_images) {
  ...
  XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.NET_WM_ICON, XA_CARDINAL, 32, PropModeReplace,
...
_SOKOL_PRIVATE void _sapp_x11_create_window(Visual* visual, int depth) {
  ...
  _sapp_x11_grab_error_handler();
  _sapp.x11.window = XCreateWindow(_sapp.x11.display, _sapp.x11.root, window_xpos, window_ypos, (uint32_t)window_width,
  ...
  if (_sapp.drop.enabled) {
    ...
    XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.xdnd.XdndAware, XA_ATOM, 32, PropModeReplace,
...
_SOKOL_PRIVATE unsigned long _sapp_x11_get_window_property(Window window, Atom property, Atom type,
                                                           unsigned char** value) {
  Atom actualType;
  int actualFormat;
  unsigned long itemCount, bytesAfter;
  XGetWindowProperty(_sapp.x11.display, window, property, 0, LONG_MAX, False, type, &actualType, &actualFormat,
...
_SOKOL_PRIVATE int _sapp_x11_get_window_state(void) {
  ...
  if (_sapp_x11_get_window_property(_sapp.x11.window, _sapp.x11.WM_STATE, _sapp.x11.WM_STATE,
...
_SOKOL_PRIVATE void _sapp_x11_process_event(XEvent* event) {
  ...
  case ClientMessage:
    ...
    if (event->xclient.message_type == _sapp.x11.WM_PROTOCOLS) {
      ...
      Atom* formats = 0;
      if (is_list) {
        count = _sapp_x11_get_window_property(_sapp.x11.xdnd.source, _sapp.x11.xdnd.XdndTypeList, XA_ATOM,
      ...
      if (_sapp.x11.xdnd.format) {
        ...
        XConvertSelection(_sapp.x11.display, _sapp.x11.xdnd.XdndSelection, _sapp.x11.xdnd.format,
                          _sapp.x11.xdnd.XdndSelection, _sapp.x11.window, time);
  ...
  case SelectionNotify:
    if (event->xselection.property == _sapp.x11.xdnd.XdndSelection) {
      char* data = 0;
      uint32_t result = _sapp_x11_get_window_property(event->xselection.requestor, event->xselection.property,
...
SOKOL_API_IMPL void sapp_html5_fetch_dropped_file(const sapp_html5_fetch_request* request) {
  ...
  if (SAPP_HTML5_FETCH_ERROR_NO_ERROR != error_code) {
    _sapp_emsc_invoke_fetch_cb(index,
                               ...
                               0, // fetched_size
                               request->buffer_ptr, request->buffer_size, request->user_data);
  } else {
    sapp_js_fetch_dropped_file(index, request->callback, request->buffer_ptr, request->buffer_size, request->user_data);
  }
#else
  (void)request;
#endif
