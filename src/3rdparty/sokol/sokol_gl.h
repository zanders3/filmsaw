...
SOKOL_GL_API_DECL void sgl_point_size(float s, float w);
SOKOL_GL_API_DECL void sgl_frag_size(float w, float h);// used for rounded rect shader hack
...
static void _sgl_init_pipeline(sgl_pipeline pip_id, const sg_pipeline_desc* in_desc, const sgl_context_desc_t* ctx_desc) {
    ...
    }
    {
        ...
        psize->format = SG_VERTEXFORMAT_FLOAT2;
    }
...
static void _sgl_init_context(sgl_context ctx_id, const sgl_context_desc_t* in_desc) {
    ...
    // default state
    ctx->rgba = 0xFFFFFFFF;
    ctx->point_size[0] = 1.0f; ctx->point_size[1] = 0.0f;
    ctx->frag_size[0] = 0.0f; ctx->frag_size[1] = 0.0f;
...
static inline void _sgl_vtx(_sgl_context_t* ctx, float x, float y, float z, float u, float v, uint32_t rgba) {
    ...
    if (vtx) {
        ...
        vtx->uv[0] = u; vtx->uv[1] = v; vtx->uv[2] = ctx->frag_size[0]; vtx->uv[3] = ctx->frag_size[1];
        vtx->rgba = rgba;
        vtx->psize[0] = ctx->point_size[0]; vtx->psize[1] = ctx->point_size[1];
    }
    ctx->vtx_count++;
}
...
SOKOL_API_IMPL void sgl_defaults(void) {
    ...
    ctx->rgba = 0xFFFFFFFF;
    ctx->point_size[0] = 1.0f;
    ctx->point_size[1] = 0.0f;
...
SOKOL_API_IMPL void sgl_point_size(float s, float w) {
    _sgl_context_t* ctx = _sgl.cur_ctx;
    if (ctx) {
        ctx->point_size[0] = s;
        ctx->point_size[1] = w;
...
SOKOL_API_IMPL void sgl_frag_size(float w, float h) {
    _sgl_context_t* ctx = _sgl.cur_ctx;
    if (ctx) {
        ctx->frag_size[0] = w; ctx->frag_size[1] = h;
    }
}
